...
export default function CustomerTicketManagement() {
  ...
  const s = {
    ...
    badge: (status) => ({
      ...
      backgroundColor: status === "Closed" ? "rgba(100, 116, 139, 0.2)" : status === "Resolved" ? "rgba(16, 185, 129, 0.2)" : "rgba(59, 130, 246, 0.2)",
      color: status === "Closed" ? "#94a3b8" : status === "Resolved" ? "#10b981" : "#3b82f6",
    })
  };
    ...
    <div style={s.bg}>
      <div style={{ ...s.panel, width: "35%", minWidth: "320px", height: "calc(100vh - 48px)" }}>
            ...
            {["All", "Open", "Resolved", "Closed"].map(st => (
              <button key={st} onClick={() => setFilterStatus(st)}
                style={{ flex: 1, padding: "6px 0", borderRadius: "6px", fontSize: "12px", cursor: "pointer", border: "none", fontWeight: "600",
                  backgroundColor: filterStatus === st ? "#f97316" : "#1f2937", color: filterStatus === st ? "white" : "#9ca3af" }}>
                {st}
              </button>
            ))}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px" }}>
          {isLoading ? <div style={{ textAlign: "center", color: "#6b7280", marginTop: "40px" }}>Đang tải...</div> : 
      ...
      <div style={{ ...s.panel, flex: 1, height: "calc(100vh - 48px)" }}>
        {!selectedTicket ? (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", color: "#4b5563" }}><h3>Chọn một Ticket để xem phản hồi</h3></div>
        ) : (
          <>
            <div style={{ padding: "20px", borderBottom: "1px solid #1f2937", display: "flex", justifyContent: "space-between", backgroundColor: "#111827", alignItems: "center" }}>
              <div>
                <h2 style={{ fontSize: "18px", fontWeight: "700", margin: "0 0 4px 0", color: "#f9fafb" }}>{selectedTicket.subject}</h2>
                <span style={{ fontSize: "13px", color: "#9ca3af" }}>Shop: <strong style={{color: "#f97316"}}>{selectedTicket.vendorName}</strong></span>
              </div>
                  ...
                  <div key={msg.messageId} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}>
                    ...
                    {msg.messageContent && (
                      <div style={{ 
                        ...
                        backgroundColor: isMine ? "#3b82f6" : "#1f2937", // Default màu xanh dịu mắt
                        color: "white", 
            ...
            <div style={{ padding: "20px", borderTop: "1px solid #1f2937" }}>
              {selectedTicket.status === "Closed" ? (
                ...
                <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
                  ...
                  <button onClick={handleSendReply} disabled={!replyText.trim() && !replyFile} style={{ padding: "12px 24px", borderRadius: "8px", backgroundColor: (!replyText.trim() && !replyFile) ? "#1f2937" : "#f97316", color: (!replyText.trim() && !replyFile) ? "#4b5563" : "white", border: "none", cursor: (!replyText.trim() && !replyFile) ? "not-allowed" : "pointer", fontWeight: "600" }}>Gửi 📤</button>
                </div>
              )}
            </div>
          </>
