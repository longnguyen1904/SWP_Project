...
export default function VendorWallet() {
  ...
  const fetchWallet = useCallback(async () => {
    try {
      const res = await api.get(
        `/api/vendor/wallet?page=0&size=${PAGE_SIZE_WALLET}`,
      );
  ...
  const handleRequestPayout = async (e) => {
    ...
    if (numAmount < MIN_AMOUNT)
      return setMessage({
        text: "Minimum withdrawal is 5,000₫",
        type: "error",
      });
    if (numAmount > MAX_AMOUNT)
      return setMessage({
        text: "Maximum withdrawal is 1,000,000,000₫",
        type: "error",
      });
    ...
    try {
      ...
      setMessage({
        text: `Payout request #${res.data?.data?.payoutId || ""} submitted. Awaiting approval.`,
        type: "success",
      });
      setAmount("");
      fetchWallet();
      ...
      setMessage({
        text: String(err.response?.data?.message || "System error"),
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ color: C.textMain, fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        ...
        <div style={cardStyle}>
          <div style={labelStyle}>Available Withdrawal</div>
          <div style={{ ...valueStyle, color: C.info }}>
