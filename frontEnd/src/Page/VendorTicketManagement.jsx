  ...
  return (
    <div style={s.bg}>
      ...
      <div style={{ ...s.panel, flex: 1, height: "calc(100vh - 48px)" }}>
        {!selectedTicket ? (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", color: "#71717a" }}><h3>Chọn một Ticket để bắt đầu hỗ trợ</h3></div>
        ) : (
          <>
            <div style={{ padding: "20px", borderBottom: "1px solid rgba(63, 63, 70, 0.4)", display: "flex", justifyContent: "space-between", backgroundColor: "transparent" }}>
              <div>
                  ...
                  <div key={msg.messageId} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}>
                    ...
                    {msg.messageContent && (
                      <div style={{ 
                        ...
                        backgroundColor: isMine ? "#3b82f6" : "rgba(39, 39, 42, 0.8)", 
                        color: "white", 
                    ...
                    {msg.attachmentUrl && (
                      <div style={{ 
                        ...
                        {msg.attachmentUrl.match(/\.(jpeg|jpg|gif|png)$/i) != null ? (
                          ...
                          <a href={`http://localhost:8081${msg.attachmentUrl}`} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", padding: "12px 16px", color: "white", textDecoration: "none", fontSize: "13px", backgroundColor: isMine ? "#3b82f6" : "rgba(39, 39, 42, 0.8)" }}>
                            📎 Xem file đính kèm
                          </a>
                        )}
                      </div>
            ...
            <div style={{ padding: "20px", borderTop: "1px solid rgba(63, 63, 70, 0.4)", backgroundColor: "transparent" }}>
              {selectedTicket.status === "Closed" ? (
                ...
                <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
                  ...
                  <button onClick={handleSendReply} disabled={!replyText.trim() && !replyFile} style={{ padding: "12px 24px", borderRadius: "8px", backgroundColor: (!replyText.trim() && !replyFile) ? "rgba(39, 39, 42, 0.8)" : "#3b82f6", color: (!replyText.trim() && !replyFile) ? "#71717a" : "white", border: "none", cursor: (!replyText.trim() && !replyFile) ? "not-allowed" : "pointer", fontWeight: "600" }}>Gửi 📤</button>
                </div>
              )}
            </div>
          </>
